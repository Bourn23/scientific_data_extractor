...
## V8->V9: Better provenance (what text is used for measurements), better log-\sigma handling.
...
from pipeline_logger import StructuredLogger
import numpy as np
...
async def safe_text_call_with_retry(sec, client, model_name, sem, timeout=150, max_retries=3, paper_context=None, logger=None):
    async with sem:
        for attempt in range(max_retries):
            try:
                ...
                if logger:
                    logger.log("text", sec.title, "success", retry_count=attempt)
                ...
                if attempt == max_retries - 1:
                    if logger:
                        logger.log("text", sec.title, "failed", error_type="timeout",
                                   retry_count=attempt,
                                   extra={"section_title": sec.title, "section_content": sec.content})
                    return None, False
                ...
                if "503" in err_str or "overloaded" in err_str:
                    ...
                    if logger:
                        logger.log("text", sec.title, "failed", error_type="exception",
                                   error_msg=str(e), retry_count=attempt,
                                   extra={"section_title": sec.title, "section_content": sec.content})
                    break
        if logger:
            logger.log("text", sec.title, "failed", error_type="all_retries_exhausted",
                       retry_count=max_retries,
                       extra={"section_title": sec.title, "section_content": sec.content})
        return None, False

async def safe_image_call_with_retry(img_path, context, client, model_name, sem, sf_parser=None, timeout=180, max_retries=3, logger=None):
    """
    ...
    async with sem:
        try:
            ...
            if raw_response:
                tracker.track(raw_response, model_name)
            if logger:
                logger.log("image", img_path.name, "success" if success else "empty",
                           extra={"image_path": str(img_path), "caption": context.get("caption", "")})
            ...
            if logger:
                logger.log("image", img_path.name, "failed", error_type="timeout",
                           extra={"image_path": str(img_path), "caption": context.get("caption", "")})
            return [], False
            
async def safe_table_call_with_retry(table_data, client, model_name, sem, timeout=180, max_retries=3, paper_context=None, logger=None):
    async with sem:
        for attempt in range(max_retries):
            try:
                result, raw_response, success = await asyncio.wait_for(
                ...
                if raw_response:
                    tracker.track(raw_response, model_name)
                if logger:
                    logger.log("table", table_data.get("caption", "unknown"), "success", retry_count=attempt)
                return result, success
                ...
                if attempt == max_retries - 1:
                    if logger:
                        logger.log("table", table_data.get("caption", "unknown"), "failed",
                                   error_type="timeout", retry_count=attempt,
                                   extra={"table_caption": table_data.get("caption", ""),
                                          "table_content": table_data.get("content", ""),
                                          "table_id": table_data.get("tab_id", "")})
                    return None, False
                ...
                if "503" in err_str or "overloaded" in err_str:
                    ...
                    if logger:
                        logger.log("table", table_data.get("caption", "unknown"), "failed",
                                   error_type="exception", error_msg=str(e), retry_count=attempt,
                                   extra={"table_caption": table_data.get("caption", ""),
                                          "table_content": table_data.get("content", ""),
                                          "table_id": table_data.get("tab_id", "")})
                    break
        if logger:
            logger.log("table", table_data.get("caption", "unknown"), "failed",
                       error_type="all_retries_exhausted", retry_count=max_retries,
                       extra={"table_caption": table_data.get("caption", ""),
                              "table_content": table_data.get("content", ""),
                              "table_id": table_data.get("tab_id", "")})
...
_MAX_TABLE_DATA_ROWS = 5  # tables with more data rows than this are chunked


def _chunk_markdown_table(content: str, max_data_rows: int = _MAX_TABLE_DATA_ROWS) -> list:
    """
    Split a large markdown table into smaller chunks that share the same header block.

    Each chunk = (all header rows) + (separator line) + (any sub-header rows detected)
    + (up to max_data_rows actual data rows).

    Sub-header rows are leading content rows (after the separator) that contain no
    measurement-like numbers — e.g. a second header row like "| Sample | Ea | σ0 | σRT |"
    in a multi-level-header table.  We include these in every chunk so the LLM always
    has full column context.

    Returns:
        A list of markdown strings.  If the table fits within max_data_rows, returns
        the original content unchanged in a 1-element list.
    """
    import re as _re

    lines = [l for l in content.strip().splitlines() if l.strip()]

    # Locate the separator line: the one whose non-pipe chars are all '-' or ':'
    sep_idx = None
    for i, line in enumerate(lines):
    ...
    header_lines = lines[: sep_idx + 1]  # column headers + separator
    data_lines   = lines[sep_idx + 1:]   # everything after separator

    # Detect sub-header rows: leading data rows with no numeric measurement values.
    # A row counts as a sub-header if it has NO: decimals, scientific notation,
    # or large integers that look like data values.
    _has_measurement = _re.compile(
        r"\b\d[\d.]*\s*[×xX]\s*10"   # e.g. 6.2 × 10
        r"|\d[\d.]*\s*[eE][+-]?\d+"  # e.g. 6.2e-3
        r"|\|\s*\d+\.\d+"            # | 0.33  (decimal in a cell)
        r"|\|\s*\d{2,}"              # | 103   (multi-digit integer in a cell)
    )
    n_sub = 0
    for row in data_lines[:3]:  # inspect at most the first 3 rows
        if _has_measurement.search(row):
            break
        n_sub += 1

    full_header = header_lines + data_lines[:n_sub]
    actual_data = data_lines[n_sub:]

    if len(actual_data) <= max_data_rows:
        return [content]  # Small enough — no chunking needed

    chunks = []
    for start in range(0, len(actual_data), max_data_rows):
        chunk_lines = full_header + actual_data[start : start + max_data_rows]
        chunks.append("\n".join(chunk_lines))
    return chunks
...
async def process_table_node(client, model, table_data: dict, max_retries: int = 3, paper_context: dict = None):
    ...
    Large tables are automatically split into row-chunks to avoid JSON truncation.
    """
    # ---- Chunking: split large tables to avoid JSON output truncation ----
    chunks = _chunk_markdown_table(table_data.get("content", ""))
    if len(chunks) > 1:
        caption = table_data.get("caption", "Table")
        print(f"   📋 {caption}: Large table ({len(chunks)} chunks of ≤{_MAX_TABLE_DATA_ROWS} rows)")
        all_measurements = []
        for i, chunk_content in enumerate(chunks, 1):
            chunk_data = {**table_data, "content": chunk_content}
            chunk_result, _, chunk_ok = await process_table_node(
                client, model, chunk_data,
                max_retries=max_retries, paper_context=paper_context,
            )
            n = len(chunk_result.measurements) if chunk_result else 0
            print(f"      ↳ Chunk {i}/{len(chunks)}: {n} measurements")
            if chunk_ok and chunk_result and chunk_result.measurements:
                all_measurements.extend(chunk_result.measurements)
        return ExtractionResult(measurements=all_measurements), None, bool(all_measurements)
    ...
    **IMPORTANT INSTRUCTIONS:**
    ...
    13. IMPORTANT — output size: set `source_paragraph_text` to [] and `source_paragraph_indices` to []
        for all table-extracted measurements. Do NOT copy the table content into these fields.
...
async def run_pipeline(markdown_file, asset_dir, model):
    ...
    try:
        ...
        logger = StructuredLogger(FILE_DIR / "extraction_log.jsonl")
        logger.log("pipeline", markdown_file.name, "started")
        ...
        for sec in sections:
            if any(skip in sec.title.lower() for skip in skip_sections):
                ...
                continue
            if PROCESS_TEXT:
                tasks.append(safe_text_call_with_retry(sec, client, TEXT_MODEL, sem, paper_context=paper_context, logger=logger))
        ...
        for img_path in sorted(img_files):
            ...
            if PROCESS_IMAGE:
                tasks.append(safe_image_call_with_retry(img_path, context, client, VISION_MODEL, sem, sf_parser=sf_parser, logger=logger))
        ...
        for table_info in all_tables:
            ...
            for sec in sections:
                if table_info in sec.tables:
                    ...
                    break
            if PROCESS_TABLE:
                tasks.append(safe_table_call_with_retry(table_data, client, TEXT_MODEL, sem, paper_context=paper_context, logger=logger))
        ...
        logger.log("pipeline", markdown_file.name, "completed",
                   extra={"extracted_count": len(all_measurements), "failed_items": failed_count})
